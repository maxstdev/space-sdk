...
public class PovManagerEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (atm == null || atn == null)
        {
            Debug.LogWarning("Atm or Atn are set");
            return;
        }
        ...
        if (GUILayout.Button("Create All POVs"))
        {
			...
			mapObject.AddComponent<SpotController>();
            ...
            for (var i = 0; i < atn.ImageFileNames.Length; ++i)
            {
                ...
                if (viewMatrix.ValidTRS())
                {
                    ...
                    pc.PlaceIndicator();
                    pc.imageFileName = Path.GetFileNameWithoutExtension(imageFileName);
