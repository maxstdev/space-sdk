...
namespace maxstAR
{
	...
	public class ARFrame
	{
		...
		public Matrix4x4 GetTransform()
		{
			...
			Matrix4x4 targetPose = MatrixUtils.GetUnityPoseOriginalMatrix(pose);
			...
			m = Matrix4x4.TRS(new Vector3(0, 0, 0), rotation, new Vector3(1, 1, 1));
			convertedMatrix = convertedMatrix * m;

			rotation = MatrixUtils.QuaternionFromMatrix(convertedMatrix);
		...
		public float[] GetOriginalTransform()
		{
			float[] pose = new float[16];
			NativeAPI.maxst_ARFrame_getOriginalTransform(arFrameCPtr, pose);
			return pose;
		}
